...
import { BasePaginator, IPaginatorConfig } from 'src/shared/paginator/pagination';
...
export class Paginator<E = any> extends BasePaginator {
    ...
    private addPagination() {
        ...
        if (exist) {
            if (this.useTakeSkip) {
                void this.queryBuilder.skip(this.pagination.getOffset()).take(this.pagination.getLimit());
            } else {
                void this.queryBuilder.offset(this.pagination.getOffset()).limit(this.pagination.getLimit());
            }
        }
    }
}
