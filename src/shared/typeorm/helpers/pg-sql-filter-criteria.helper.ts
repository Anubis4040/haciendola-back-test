...
import { Parse, PrototypeToString, StringPrototypes } from 'src/shared/app/utils';
import { MapCriteria } from 'src/shared/paginator';
    ...
    filterInArrayString(attribute: Omit<AttributeConfig<F, E>, 'toLower' | 'isBoolean'> | KeyAttribute<F>, condition: FilterCondition, alias = 'i'): void {
        ...
        if (this._filter.has(_attribute)) {
            ...
            this.queryBuilder[condition](
                `EXISTS ( SELECT * FROM unnest(string_to_array("${alias}"."${_dbAttribute}", ',') ) as datarray WHERE datarray = ANY(ARRAY[${valueAttribute}]))`,
            );
        }
    }
